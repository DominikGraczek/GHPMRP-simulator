...
function computeMRP(
  ...
  for (let i = 0; i < periods; i++) {
    ...
    if (diff < 0) {
      const deficit = -diff;
      netReq[i] = deficit;
      plannedOrderReceipts[i] = lotSize;
      if (i - leadTime >= 0) {
        plannedOrderRelease[i - leadTime] = lotSize;
      }
      currentOnHand = available + lotSize - grossReq[i];
    } else {
      netReq[i] = 0;
      currentOnHand = diff;
    }
...
const MultiLevelMRP: React.FC = () => {
  const periods = 10;
  const [ghpDemand, setGhpDemand] = useState<number[]>(Array(periods).fill(0));
  ...
  const [blatScheduledReceiptsInput, setBlatScheduledReceiptsInput] = useState<
    number[]
  >(Array(periods).fill(0));
  ...
  return (
    <div className="bg-gray-900 text-white min-h-screen p-4 space-y-8">
      <div className="overflow-x-auto max-w-6xl mx-auto">
        <h2 className="text-2xl font-bold text-center mb-4">GHP (Poziom 0)</h2>
        <table className="w-full border border-gray-700 text-sm">
          <thead className="bg-gray-800">
            <tr>
              <th className="border border-gray-700 px-2 py-1">Tydzień</th>
              {Array.from({ length: periods }).map((_, i) => (
                <th key={i} className="border border-gray-700 px-2 py-1">
                  {i + 1}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            <tr>
              ...
              {ghpDemand.map((val, i) => (
                <td key={i} className="border border-gray-700 px-2 py-1">
                  <input
                    type="number"
                    value={displayVal(val)}
                    onChange={(e) => handleGhpDemandChange(i, e)}
              ...
              {ghpProduction.map((val, i) => (
                <td key={i} className="border border-gray-700 px-2 py-1">
                  <input
                    type="number"
                    value={displayVal(val)}
                    onChange={(e) => handleGhpProductionChange(i, e)}
                    className="w-full bg-gray-800 border border-gray-600 rounded px-1"
                  />
                </td>
              ))}
      ...
      <div className="overflow-x-auto max-w-6xl mx-auto">
        ...
        <table className="w-full border border-gray-700 text-sm">
          <thead className="bg-gray-800">
            <tr>
              <th className="border border-gray-700 px-2 py-1">Okres</th>
              {Array.from({ length: periods }).map((_, i) => (
                <th key={i} className="border border-gray-700 px-2 py-1">
                  {i + 1}
                </th>
              ...
              {blatMRP.scheduledReceipts.map((val, i) => (
                <td key={i} className="...">
                  <input
                    ...
                    value={val === 0 ? "" : val}
                    onChange={(e) => {
                      const newArr = [...blatScheduledReceiptsInput];
                      newArr[i] = Number(e.target.value);
                      setBlatScheduledReceiptsInput(newArr);
                    }}
            ...
            <tr>
              ...
              >
                ...
                <input
                  type="number"
                  value={leadTimeBlat}
                  onChange={(e) => setLeadTimeBlat(Number(e.target.value))}
                  className="bg-gray-800 border border-gray-600 rounded px-1 w-16"
                ></input>
                ...
                <input
                  type="number"
                  value={lotSizeBlat}
                  onChange={(e) => setLotSizeBlat(Number(e.target.value))}
                  className="bg-gray-800 border border-gray-600 rounded px-1 w-16"
                ></input>
              ...
              {kolkaMRP.scheduledReceipts.map((val, i) => (
                <td key={i} className="...">
                  <input
                    ...
                    value={val === 0 ? "" : val}
                    onChange={(e) => {
                      const newArr = [...kolkaScheduledReceiptsInput];
                      newArr[i] = Number(e.target.value);
                      setKolkaScheduledReceiptsInput(newArr);
                    }}
                    className="w-full bg-gray-800 border border-gray-600 rounded px-1 text-center"
                  />
                </td>
            ...
            <tr>
              ...
              >
                ...
                <input
                  type="number"
                  value={leadTimeKolka}
                  onChange={(e) => setLeadTimeKolka(Number(e.target.value))}
                  className="bg-gray-800 border border-gray-600 rounded px-1 w-16"
                ></input>
                <span className="ml-2 mr-2">Wielkość partii:</span>
                <input
                  type="number"
                  value={lotSizeKolka}
                  onChange={(e) => setLotSizeKolka(Number(e.target.value))}
                  className="bg-gray-800 border border-gray-600 rounded px-1 w-16"
                ></input>
                <span className="ml-2"> Poziom BOM = 1</span>
              </td>
              ...
              {lozyskaMRP.scheduledReceipts.map((val, i) => (
                <td key={i} className="...">
                  <input
                    ...
                    value={val === 0 ? "" : val}
                    onChange={(e) => {
                      const newArr = [...lozyskaScheduledReceiptsInput];
                      newArr[i] = Number(e.target.value);
                      setLozyskaScheduledReceiptsInput(newArr);
                    }}
                    className="w-full bg-gray-800 border border-gray-600 rounded px-1 text-center"
                  />
                </td>
            ...
            <tr>
              ...
              >
                ...
                <input
                  ...
                  onChange={(e) => setLeadTimeLozyska(Number(e.target.value))}
                  className="bg-gray-800 border border-gray-600 rounded px-1 w-16"
                ></input>
                <span className="ml-2 mr-2">Wielkość partii:</span>
                <input
                  ...
                  onChange={(e) => setLotSizeLozyska(Number(e.target.value))}
                  className="bg-gray-800 border border-gray-600 rounded px-1 w-16"
                ></input>
